...
Исходные данные загружаются из файла Excel и сортируются по коду респондента.
Для каждого респондента формируется метка страты как конкатенация его возрастной 
группы и пола:
...
Столбцы мастер-таблицы разбиваются на блоки: метаданные, оценки \textit{идеального}
 продукта, веса \textit{важности} параметров и оценки каждого из трeх брендов (Sony, JBL, Marshall).
...
Для каждого респондента и каждого бренда вычисляется интегральный балл отклонения
от идеала по формуле~\ref{eq:integral_score} из раздела методологии.
...
for name in BRAND_NAMES:
    ...
    score = (np.abs(ideal_cols - df[brand_cols]) * imp_cols).sum(axis=1)
    all_scores.append(score)

pop_scores = np.column_stack(all_scores)
\end{mintedbox}

Результатом является матрица \texttt{pop\_scores} размером $N \times 3$, где каждая строка
соответствует респонденту, а столбцы --- трeм брендам. На еe основе вычисляются \textit{истинные}
(генеральные) средние значения и определяется бренд, наиболее близкий к идеалу.
